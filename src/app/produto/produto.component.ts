...
import {
	Produto
} from '../shared/model/produto-model';
import {
	LoadingService
...
import {
	Categoria
} from '../shared/model/categoria-model';
import {
	CategoriaService
} from '../categoria/categoria.service';
import {
	Observable,
	ReplaySubject
} from 'rxjs';
...
export class ProdutoComponent implements OnInit {
	...
	deletarProdutoModal(id: string): void {
		const produtoDeletar = this.produtos.find(produto => produto.id === id);
		if (!produtoDeletar)
			return;
		this.produto = produtoDeletar;
	}
	...
	cadastrarProduto(): void {
		...
		this.service.cadastrarProduto(<Produto> this.produtoForm.getRawValue()).then((result: Produto) => {
	...
	atualizarProduto(): void {
		this.loadingService.loading = true;
		this.service.atualizarProduto(<Produto> this.produtoForm.getRawValue()).then((result: Produto) => {
			...
			console.log(error);
			this.loadingService.loading = false;
			this.toastrService.error('Erro ao editar produto!');
		});
	}

	deletarProduto(): void {
		this.loadingService.loading = true;
		this.service.deletarProduto(this.produto).then((result: Produto) => {
			...
			console.log(error);
			this.loadingService.loading = false;
			this.toastrService.error('Erro ao deletar produto!');
		});
	}

	listaProdutos(): void {
		this.loadingService.loading = true;
		this.service.listarProdutos().then((result: Produto[]) => {
			this.produtos = result;
			this.loadingService.loading = false;
		}).catch((error) => {
			console.log(error);
			this.loadingService.loading = false;
			this.toastrService.error('Erro ao listar produtos!');
		});
	}
